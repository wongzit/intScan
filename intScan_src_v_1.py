...
for j in range(1, stepNo+1):
	...
	for k in molCoor1:
#		if k[0] != 'Bq':
		xyzFile.write(f'{k[0]}                 {round(float(k[1]),8)}    {round(float(k[2]),8)}    {round(float(k[3]),8)}\n')
		if bsseFlag == 1 and k[0] != 'Bq':
			gjfFile.write(f'{k[0]}(Fragment=1)                 {round(float(k[1]),8)}    {round(float(k[2]),8)}    {round(float(k[3]),8)}\n')
		elif k[0] != 'Bq':
			gjfFile.write(f'{k[0]}                 {round(float(k[1]),8)}    {round(float(k[2]),8)}    {round(float(k[3]),8)}\n')
	for l in molCoor2:
#		if l[0] != 'Bq':
		xyzFile.write(f'{l[0]}                 {round(float(l[1]+j*stepSize*vecUnit[0]),8)}    {round(float(l[2]+j*stepSize*vecUnit[1]),8)}    {round(float(l[3]+j*stepSize*vecUnit[2]),8)}\n')
		if bsseFlag == 1 and l[0] != 'Bq':
			gjfFile.write(f'{l[0]}(Fragment=2)                 {round(float(l[1]+j*stepSize*vecUnit[0]),8)}    {round(float(l[2]+j*stepSize*vecUnit[1]),8)}    {round(float(l[3]+j*stepSize*vecUnit[2]),8)}\n')
		elif l[0] != 'Bq':
			gjfFile.write(f'{l[0]}                 {round(float(l[1]+j*stepSize*vecUnit[0]),8)}    {round(float(l[2]+j*stepSize*vecUnit[1]),8)}    {round(float(l[3]+j*stepSize*vecUnit[2]),8)}\n')
	gjfFile.write('\n\n')
	gjfFile.close()
